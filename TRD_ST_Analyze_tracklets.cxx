                            ...
                            if(flag_TRD_TPC_tracks == 0) // TRD tracks
                            {
                                ...
                                //if(fabs(AP_alpha) < 0.2 && AP_pT > 0.0 && AP_pT < 0.02 && CA*CB < 0.0 && pTA > 0.3 && pTB > 0.3 && pTA < 0.5 && pTB < 0.5 && dot_product_dir_vertex > 0.9) // TPC photon conversion
                                {
                                    ...
                                    if(graphics)
                                    {
                                        ...
                                        //PonTEveP_second_point_helix ->SetPoint(i_vertex_acc+2,helix_pointBs[0],helix_pointBs[1],helix_pointBs[2]);
...
void Ali_TRD_ST_Analyze::set_Kalman_TRD_tracklets(vector< vector<Ali_TRD_ST_Tracklets*> > vec_kalman_TRD_trackets_in)
{
    ...
    for(Int_t i_track_A = 0; i_track_A < (Int_t)vec_helices_TRD.size(); i_track_A++)
    {
        ...
        for(Int_t i_layer = 0; i_layer < 6; i_layer++)
        {
            ...
            {
#if 1
                if((Int_t)vec_helices_TRD.size() == 2 && N_good_layers == 0) // for cosmics
                {
                    TVector3 kalman_TV3_offset = vec_kalman_TRD_trackets[i_track_A][i_layer] ->get_TV3_offset();
                    //printf("i_track_A: %d   ====> offset: {%4.3f, %4.3f, %4.3f} \n",i_track_A,kalman_TV3_offset.X(),kalman_TV3_offset.Y(),kalman_TV3_offset.Z());
                }
#endif
                N_good_layers++;
            }
        }
        //if(N_good_layers > 3) printf("        -----> Found track with %d good layers \n",N_good_layers);
    }
...
void Ali_TRD_ST_Analyze::Draw_Kalman_Helix_Tracks(Int_t n_track, Int_t color, Double_t low_R, Double_t high_R)
{
    ...
    for(Int_t i_track = i_track_start; i_track < i_track_stop; i_track++)
    {
        ...
        Double_t radius_old   = 10000.0;
        for(Double_t track_path = 75.0; track_path > -750.0; track_path -= 1.0)  // -350.0
        {
            Evaluate(track_path,track_pos);
            radius_helix = TMath::Sqrt( TMath::Power(track_pos[0],2) + TMath::Power(track_pos[1],2) );
            if(radius_helix > radius_old) break;
            radius_old = radius_helix;
...
void Ali_TRD_ST_Analyze::Init_tree(TString SEList)
{
    ...
    TRD_ST_Digit      = new Ali_TRD_ST_Digits();
...
Int_t Ali_TRD_ST_Analyze::Loop_event(Long64_t i_event, Int_t graphics)
{
    ...
    //for(Int_t i_tracklet = 0; i_tracklet < 10; i_tracklet++)
    {
        ...
        //printf("i_tracklet: %d,  pos A: {%4.2f, %4.2f, %4.2f}, trk_index: %d \n",i_tracklet,TV3_offset[0],TV3_offset[1],TV3_offset[2],trk_index);
...
void Ali_TRD_ST_Analyze::Scan_MC_Event(Int_t graphics, Int_t bool_make_invariant_mass_hist_up_to_layer)
{
    ...
    { 
        ...
        vec_inv_masses_per_generation_reconstructed.resize(bool_make_invariant_mass_hist_up_to_layer+1);
                ...
                for (auto const &indVector : vec_indices_per_generation) {
                    ...
                    if (it != std::end(indVector)) {
                        ...
                        if(possible_reconstructed_mass ){
                            if(map_matched_mc_to_tpc.find(MC_index_particle) != map_matched_mc_to_tpc.end() ){
                                TRD_ST_TPC_Track = TRD_ST_Event ->getTrack(map_matched_mc_to_tpc[MC_index_particle]);
                                TLorentzVector TLV_MC_particle_rec = TRD_ST_TPC_Track ->get_TLV_part();
                                TLV_MC_particle_rec.SetPtEtaPhiM(TLV_MC_particle_rec.Pt(),TLV_MC_particle_rec.Eta(),TLV_MC_particle_rec.Phi(),PDG_masses[MC_PDG_code]);

                                vec_inv_masses_per_generation_reconstructed[pos_index].push_back(TLV_MC_particle_rec);
                                
                            }
                            else if(TMath::Abs(MC_PDG_code) == 211 || TMath::Abs(MC_PDG_code) == 2212 || TMath::Abs(MC_PDG_code) == 321 || pos_index == bool_make_invariant_mass_hist_up_to_layer){
                                possible_reconstructed_mass = kFALSE;
                            }
                            else{
                                vec_inv_masses_per_generation_reconstructed[pos_index].push_back(TLorentzVector());
                            }
                        }
    ...
    if(bool_make_invariant_mass_hist_up_to_layer){
        for(Int_t i_gen = bool_make_invariant_mass_hist_up_to_layer; i_gen >= 1; i_gen--)
        {
            for (Int_t i_particle = 0; i_particle < (Int_t) vec_indices_per_generation[i_gen].size(); i_particle++)
            {
                ...
                if(possible_reconstructed_mass){
                    vec_inv_masses_per_generation_reconstructed[i_gen-1][pos_l_vec]+=vec_inv_masses_per_generation_reconstructed[i_gen][i_particle];  
                }
                ...
                if (TMath::Abs(TRD_MC_Track_pi0->get_PDGcode())==310){ 
                    ...
                    if(possible_reconstructed_mass)
                        h1D_invariant_mass_K_reconstructed -> Fill(vec_inv_masses_per_generation_reconstructed[i_gen][i_particle].M(), 1);
        ...
        if(possible_reconstructed_mass){
            vec_inv_masses_per_generation_reconstructed[0][0]-= TLorentzVector(0,0,0,PDG_masses[2112]);
            h1D_invariant_mass_S_reconstructed -> Fill(vec_inv_masses_per_generation_reconstructed[0][0].M(), 1);
...
void Ali_TRD_ST_Analyze::Draw_Inv_Mass_histogram(){
    ...
    TCanvas* can_invariant_mass_S_reconstructed = new TCanvas("can_invariant_mass_S_reconstructed", "can_invariant_mass_S_reconstructed", 10, 10, 500, 500);
...
void Ali_TRD_ST_Analyze::Draw_MC_track_w_vertices(Int_t i_track_print_nbr, Int_t color, Double_t line_width, Double_t max_path, Int_t i_track)
{
    ...
    {3112,-1.0},{-3112,1.0},{-3122,0},{3122,0},{3222,1.0},{3212,0},{-3212,0}};
      ...
      if(!(i_track_print_nbr)){
        ...
        pm    ->SetNextPoint(track_pos[0],track_pos[1],track_pos[2]);
        pm    ->SetName(HistName.Data());
        //pm    ->SetLineStyle(1);
        pm    ->SetMarkerSize(2);
    ...
    if(fabs(charge)<1e-8){ //neutral particle ... project a straight line
      ...
      //for(Double_t track_path = 0; track_path < 500 && dist_to_endvertex>= 10 &&  ; track_path += 10.0)
      //{
      Double_t track_path = 0;
      Bool_t continue_loop = kTRUE;
      radius_helix = 0;
      do{
        #if defined(USEEVE)
                vec_TPL3D_helix_MC[i_track]        ->SetNextPoint(track_pos[0],track_pos[1],track_pos[2]);
        ...
        track_pos[2] += velo_mc[2];
        radius_helix = TMath::Sqrt( TMath::Power(track_pos[0],2) + TMath::Power(track_pos[1],2) );
        dist_to_endvertex =TMath::Sqrt( TMath::Power(TV3_MC_particle_vertex_end.X()-track_pos[0],2) + TMath::Power(TV3_MC_particle_vertex_end.Y()-track_pos[1],2) + TMath::Power(TV3_MC_particle_vertex_end.Z()-track_pos[2],2));
        track_path+=1;
        ...
        Bool_t continue_loop = kTRUE;
        radius_helix = 0;
        //for(Double_t track_path = 0; dist_to_endvertex >= 1.0 ; track_path += 1.0)
        //{
        TRD_ST_TPC_Track_MC ->Evaluate(track_path,track_pos);
        track_path+=1;
        do{
    ...
    vec_TPL3D_helix_MC[i_track]    ->SetLineStyle(1);
    vec_TPL3D_helix_MC[i_track]    ->SetLineWidth(line_width);
    vec_TPL3D_helix_MC[i_track]    ->SetMainColor((abs(MC_PDG_code+1)*1347 +46573)%120 +791);
    vec_TPL3D_helix_MC[i_track]    ->SetMainAlpha(1.0);
...
Int_t Ali_TRD_ST_Analyze::Draw_event(Long64_t i_event, Int_t graphics, Int_t draw_tracks, Int_t draw_tracklets, Double_t track_path, Int_t draw_digits)
{
...
#if defined(USEEVE)
    if(graphics && draw_digits)
    {
        TEveP_digit_positions        = new TEvePointSet();
        Int_t    NumDigits         = TRD_ST_Event ->getNumDigits();
        TVector3 TV3_digit_pos;
        for(Int_t i_digit = 0; i_digit < NumDigits; i_digit++)
        {
            TRD_ST_Digit  = TRD_ST_Event ->getDigit(i_digit);
            TV3_digit_pos = TRD_ST_Digit ->get_TV3_position();

            TEveP_digit_positions ->SetPoint(i_digit,TV3_digit_pos.X(),TV3_digit_pos.Y(),TV3_digit_pos.Z());
            TEveP_digit_positions ->SetMarkerSize(1);
            TEveP_digit_positions ->SetMarkerStyle(20);
            TEveP_digit_positions ->SetMarkerColor(kRed+1);
        }
        gEve->AddElement(TEveP_digit_positions);
    }
#endif
    ...
    TVector3 TV3_dir;
    Int_t    i_det, trk_index;
    Double_t ADC_val_q0, ADC_val_q1, ADC_val_q2;
    ...
    for(Int_t i_tracklet = 0; i_tracklet < NumTracklets; i_tracklet++)
    {
        ...
        i_det           = TRD_ST_Tracklet ->get_TRD_det();
        trk_index       = TRD_ST_Tracklet ->get_TRD_index();
        ADC_val_q0      = TRD_ST_Tracklet ->get_ADC_val(0);
        ADC_val_q1      = TRD_ST_Tracklet ->get_ADC_val(1);
        ADC_val_q2      = TRD_ST_Tracklet ->get_ADC_val(2);
    ...
    for(Int_t i_Track=0;i_Track<(Int_t)found_tracks.size();i_Track++)
    {
        ...
        for(Int_t i_lay=0;i_lay<6;i_lay++)
        {
            if(found_tracks[i_Track][i_lay]!=NULL)
            {
                ...
                Double_t size_tracklet = found_tracks[i_Track][i_lay]->get_TV3_dir().Mag();
                //printf("size_tracklet: %4.3f \n",size_tracklet);

                Int_t trk_index = found_tracks[i_Track][i_lay]->get_TRD_index();
                ...
                if(size_tracklet > 1.5) TEveLine_Kalman_found[i_Track][i_lay]   ->SetMainColor(kBlue);
